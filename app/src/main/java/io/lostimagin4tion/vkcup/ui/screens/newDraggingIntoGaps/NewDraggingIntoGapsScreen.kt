...
/**
 * [NewDraggingIntoGapsScreen] - screen for interactive element that will work like dragging options
 * into gaps in the text
 *
 * @author Egor Danilov
 */
@Composable
fun NewDraggingIntoGapsScreen(
    postViewModel: WritePostViewModel,
    navController: NavHostController,
    showMessage: (Int) -> Unit
) {
    val viewModel: NewDraggingIntoGapsViewModel = viewModel()
    val quizData by viewModel.quizData.collectAsState()

    NewDraggingIntoGapsScreenContent(
        quizData = quizData,
        ...
        finishCreatingDragging = postViewModel::addInteractiveElement,
        navigateBack = navController::popBackStack
    )
}

/**
 * [NewDraggingIntoGapsScreenContent] - layout of [NewDraggingIntoGapsScreen]
 *
 * @author Egor Danilov
 */
@Composable
private fun NewDraggingIntoGapsScreenContent(
    ...
    deleteGap: (Int) -> Unit = {},
    finishCreatingDragging: (InteractiveElementData, Int) -> Unit = { _, _ -> },
    navigateBack: () -> Unit = {}
) = Column(
    ...
    val onContinueButtonClick: () -> Unit = {
        ...
        if (questionInput.text.isBlank()) {
            showMessage(R.string.dragging_creation_empty_question_error)
        }
        else if (quizData.gaps.isEmpty()) {
            showMessage(R.string.dragging_creation_no_gaps_error)
        }
        else {
            finishCreatingDragging(quizData, getRememberedIndex())
            navigateBack()
        }
    }
    ...
    AppBarWithMultipleButtons(
        title = {
            SubtitleText(
                textRes = R.string.new_dragging_header,
                isLarge = false,
        ...
        onTrailingIconsClick = listOf(
            { showMessage(R.string.dragging_hint) },
    ...
    Column(
        ...
        CustomOutlinedTextField(
            ...
            minHeight = 200.dp,
            placeholderRes = R.string.dragging_question_placeholder,
        ...
        TextFilledButton(
            ...
            textResource = R.string.add_dragging_variant_button,
            modifier = Modifier
                .fillMaxWidth()
                .padding(
...
private fun NewMatchingScreenPreview() = VkCupTheme {
    val data by remember {
        mutableStateOf(QuizWithGaps(interactionMethod = GapsInteractionMethod.Dragging))
    }

    NewDraggingIntoGapsScreenContent(data)
}